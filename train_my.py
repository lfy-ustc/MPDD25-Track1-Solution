...
def eval_ensemble(model_list, weights, val_loader, device):
    assert len(model_list) == len(weights), "模型数和权重数不一致"
    # print(model_list)
    ...
    with torch.no_grad():
        for data in val_loader:
            ...
            for model in model_list:
                ...
                model.test()
                logits = model.emo_pred.detach()  # shape: [B, num_classes]
                pred_logits_list.append(logits)

            # 加权平均 soft voting
            probs_list = [torch.softmax(logits, dim=-1) for logits in pred_logits_list]  # 转为概率
            stacked_probs = torch.stack(probs_list, dim=0)  # [num_models, B, num_classes]
            weights_tensor = torch.tensor(weights, device=stacked_probs.device).view(-1, 1, 1)  # [num_models,1,1]
            weighted_probs = (stacked_probs * weights_tensor).sum(dim=0)  # [B, num_classes]


            emo_pred = weighted_probs.argmax(dim=1).cpu().numpy()
            emo_label = data['emo_label'].cpu().numpy()

            total_emo_preds.append(emo_pred)
            total_emo_labels.append(emo_label)
            person_id_list.extend(data['person_id'])

    total_emo_pred = np.concatenate(total_emo_preds)
    total_emo_label = np.concatenate(total_emo_labels)

    emo_acc_unweighted = accuracy_score(total_emo_label, total_emo_pred)
...
def eval(model, val_loader, device):
    ...
    with torch.no_grad():
        for data in val_loader:
            ...
            model.set_input(data)
            model.test()
            # total_hidden_state.append(model.hidden_state.detach().cpu())
            # total_file_name_list.append(f"{data['person_id'][0]}_{data['chunk_id'][0]}")
            emo_pred = model.emo_pred.argmax(dim=1).cpu().numpy()
            emo_label = data['emo_label'].cpu().numpy()
            total_emo_pred.append(emo_pred)
            total_emo_label.append(emo_label)
            person_id_list.extend(data['person_id'])
        # total_feature = torch.cat(total_hidden_state,dim=0)
    # features_np = total_feature.cpu().numpy()
    # df = pd.DataFrame(features_np)
    # df.insert(0, "ID", total_file_name_list)  # 第一列插入样本 ID
    # df["Label"] = [int(label[0]) for label in total_emo_label]  # 最后一列插入标签
...
def train_model(train_json, model, audio_path='', video_path='', max_len=5,
    ...
    for epoch in range(num_epochs):
        model.train(True)
        total_loss = 0
        if opt.longtail and epoch == reweight_epoch:
            model.set_criterion()
        total_emo_CE = 0
        total_emo_kl = 0
        total_EmoF_CE = 0
        ...
        metric = (emo_acc_weighted+emo_acc_unweighted+emo_f1_weighted+emo_f1_unweighted)/4
...
if __name__ == '__main__':
    ...
    args.train_json = os.path.join(args.data_rootpath, 'Training', 'labels', 'Training_Validation_label_files.json')
