...
class ourModel(BaseModel, nn.Module):
    ...
    def __init__(self, opt):
        ...
        # acoustic model
        self.netEmoA = LSTMEncoder(opt.input_dim_a, opt.embd_size_a, embd_method=opt.embd_method_a,bidirectional=opt.bidirectional)
        self.model_names.append('EmoA')

        # visual model
        self.netEmoV = LSTMEncoder(opt.input_dim_v, opt.embd_size_v, opt.embd_method_v,bidirectional=opt.bidirectional)
    ...
    def forward(self, acoustic_feat=None, visual_feat=None):
        ...
        if torch.isnan(self.acoustic).any():
            # print(nan_indices.shape)
            print("input acoustic is nan")
        if torch.isnan(self.visual).any():
            print("input visual is nan")
        # print(A_valid_count, V_valid_count)
        ...
        if (V_valid_count == 0).any():
            print("Warning: V_valid_count contains zero values!")
        if (A_valid_count == 0).any(): 
            print("Warning: A_valid_count contains zero values!")
        ...
        # self.emo_logits_fuse= 0.6*self.emo_logits+0.4*self.emo_logits_fusion
        # print(self.emo_logits)
        self.emo_pred = F.softmax(self.emo_logits, dim=-1)

    def backward(self):
        """Calculate the loss for back propagation"""
        self.loss_emo_CE = self.criterion_ce(self.emo_logits, self.emo_label) 
        # print(self.loss_emo_CE)
        self.loss_EmoF_CE = self.focal_weight * self.criterion_focal(self.emo_logits_fusion, self.emo_label)
        self.loss_emo_kl = conditional_kl_div_loss(self.emo_logits, self.emo_logits_fusion, self.emo_label)
        # print(self.emo_label)
        # print(self.loss_emo_CE, self.loss_EmoF_CE, self.loss_emo_kl)
        loss = self.loss_emo_CE + self.loss_EmoF_CE + self.loss_emo_kl
...
class Focal_Loss(nn.Module):
    ...
    def __init__(self, alpha=[0.1,0.2,0.2,0.3,0.2], gamma=3, reduction='mean'): #五分类
...
def conditional_kl_div_loss(y_m1, y_m2, y_gt):
    """
    Compute conditional KL divergence loss.
    
    Args:
        y_m1: Tensor of shape (B, C) - Predictions from modality m1 (e.g., video)
        y_m2: Tensor of shape (B, C) - Predictions from modality m2 (e.g., text)
        y_gt: Tensor of shape (B,) - Ground truth labels (integer class indices)
    
    Returns:
        Loss value (scalar tensor)
    """
    # Convert ground truth labels to one-hot encoding
    y_gt_one_hot = F.one_hot(y_gt, num_classes=y_m1.shape[1]).float()
    
    # Compute argmax to find predicted class
    pred_m1 = torch.argmax(y_m1, dim=1)
    pred_m2 = torch.argmax(y_m2, dim=1)
    
    # Find where m1 is correct
    mask_m1_correct = (pred_m1 == y_gt).float().unsqueeze(1)  # Shape (B, 1)
    # Find where m2 is correct
    mask_m2_correct = (pred_m2 == y_gt).float().unsqueeze(1)  # Shape (B, 1)
    
    # Compute KL divergence only when the respective modality is correct
    kl_m1_to_m2 = F.kl_div(F.log_softmax(y_m2, dim=1), F.softmax(y_m1, dim=1), reduction='none').sum(dim=1)
    kl_m2_to_m1 = F.kl_div(F.log_softmax(y_m1, dim=1), F.softmax(y_m2, dim=1), reduction='none').sum(dim=1)
    
    # Apply conditional logic: only keep KL divergence when the correct modality exists
    loss = (mask_m1_correct.squeeze() * kl_m1_to_m2) + (mask_m2_correct.squeeze() * kl_m2_to_m1)
    # Average over batch
