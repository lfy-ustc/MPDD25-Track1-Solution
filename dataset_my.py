...
class AudioVisualDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if self.isTest == False:
            ...
            phq9_score = torch.tensor(entry['PHQ_9'], dtype=torch.long)
        else:
            label = torch.tensor(0, dtype=torch.long) 
            phq9_score = torch.tensor(0, dtype=torch.long)
        ...
        chunk_id = int(filename.split('.')[0].split('_')[2])
        ...
        # print(audio_feature.shape, video_feature.shape, label.shape, personalized_feature.shape)
        return {
    ...
    def collate_fn(self, batch):
        ...
        """
        A_feats = [item['A_feat'] for item in batch]  # List of tensors with shape (B, L, C)
        V_feats = [item['V_feat'] for item in batch]  # List of tensors with shape (B, L, C)
        
        # 获取 batch 中所有样本的 batch 维度 B
        B = A_feats[0].shape[0]  # 假设所有样本 B 维度相同

        # 按 L 维度进行填充
        A_feats_padded = pad_sequence(A_feats, batch_first=True)  # (B, max_L, C)
        V_feats_padded = pad_sequence(V_feats, batch_first=True)  # (B, max_L, C)
        A_max_len = A_feats_padded.size(1)
        V_max_len = V_feats_padded.size(1)
        min_max_len = min(A_max_len,V_max_len)
        A_feats_padded = A_feats_padded[:,:min_max_len,:]
        V_feats_padded = V_feats_padded[:,:min_max_len,:]

        A_lengths = torch.tensor([feat.shape[0] for feat in A_feats])
        mask_A =  torch.arange(min_max_len, device=A_feats_padded.device).expand(len(A_lengths), min_max_len) < A_lengths.unsqueeze(1)
        V_lengths = torch.tensor([feat.shape[0] for feat in V_feats])
        mask_V = torch.arange(min_max_len, device=V_feats_padded.device).expand(len(V_lengths), min_max_len) < V_lengths.unsqueeze(1)
        # print(A_feats_padded.shape)
        # print(V_feats_padded.shape)
        
        emo_label = torch.stack([item['emo_label'] for item in batch], dim=0)
        phq9_score = torch.stack([item['phq9_score'] for item in batch], dim=0)
        personalized_feat = torch.stack([item['personalized_feat'] for item in batch], dim=0)
        person_id = [item['person_id'] for item in batch]
        chunk_id = [item['chunk_id'] for item in batch]
        return {
            'A_feat': A_feats_padded,  # (B, max_L, C)
            'V_feat': V_feats_padded,  # (B, max_L, C)
            'A_mask': mask_A,
            "V_mask": mask_V,
            'emo_label': emo_label,
            'personalized_feat': personalized_feat,
            'seq_lengths': torch.tensor([item['A_feat'].shape[1] for item in batch]),  # 记录原始 L
            'person_id': person_id,
            'chunk_id':chunk_id,
            'phq9_score':phq9_score
        }