...
def train_val_split1(file_path, val_ratio=0.1, random_seed=3407):
    ...
    np.random.seed(random_seed)  # 确保 NumPy 相关的随机操作固定
    # print(val_ratio)
    ...
    for item in data:
        ...
        id_ = item["id"]
        if id_ not in label_to_ids[pen_category]:
            label_to_ids[pen_category].append(id_)
...
def get_data(file_path,track):
    # print(file_path)
    with open(file_path, 'r') as file:
        data = json.load(file)
    category_count = defaultdict(int)
    if track == 'Track2':
        count_key = 'tri_category'
    else:
        count_key = 'pen_category'
    for entry in data:
        category_count[entry[count_key]] += 1
